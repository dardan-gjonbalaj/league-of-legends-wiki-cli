...
class CLI
  ...
  def find_champ(name = nil)
    name = gets.strip if name.nil?
    temp = nil

    if temp = Champion.find_by_name(name.gsub(/\w+/, &:capitalize))
      if temp.abilities.empty?
        get_abilities = get_abilities(temp.name)
        temp.abilities << get_abilities
      end
  ...
  def champs_by_role
    ...
    get_roles.each.with_index(1) do |champ, index|
      puts "#{index}: #{champ.name} "
      champ.abilities << get_abilities(champ.name) if champ.abilities.empty?
    end
